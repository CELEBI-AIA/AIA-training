#!/usr/bin/env python3
"""
Checkpoint temizleme — best.pt, last.pt ve son 3 epoch checkpoint dışındakileri siler.
Cross-platform (Windows/Linux/Colab). Otomatik olarak train pipeline tarafından çağrılır.
"""
import sys
...
def cleanup_checkpoints(weights_dir: Path) -> int:
    """
    Eski epoch checkpoint'lerini sil. best.pt, last.pt ve son KEEP_EPOCHS epoch*.pt kalır.
...
def cleanup_run(run_dir: Path) -> int:
    """Bir run klasöründeki weights/ altındaki checkpoint'leri temizle."""
    weights_dir = run_dir / "weights"
    return cleanup_checkpoints(weights_dir)


def cleanup_all_runs(project_dir: Path) -> int:
    """Proje altındaki tüm run klasörlerini temizle."""
    total = 0
